...
myPvalue <- Data %>% 
  ...
  summarise(
    myP = kruskal.test(CV ~ Cell)$p.value
  )
myPvalue$padj <- p.adjust(myPvalue$myP, method = )
...
CellType <- c("NCM460", "HT29", "RKO", "HCT116")

for (celltype in CellType[2:length(CellType)]) {
  for (c in unique(Data$Concentration)) {
    meanData$myPvalue[which(meanData$Cell == celltype & meanData$Concentration == c)] <- wilcox.test(
      ...
      Data$CV[which(Data$Cell == CellType[1] & Data$Concentration == c)]
      )$p.value
  }
...
for (m in 1:length(CellType)) {
  meanData$x[which(meanData$Cell == CellType[m])] <- 1:8
}


# plot --------------------------------------------------------------------

myCol <- c('#1E90FF', '#0c8918', '#ffa631', '#6B8E23', 'white') ##  DodgerBlue #1E90FF，ForestGreen #228B22，OliveDrab #6B8E23
boxcolor <- 'red' # '#f2be45'  # ,'#ff3300'


pdf(paste0('triangle_', sub(".xlsx", "", myFiles[f]), ".pdf"), height = 5, width = 10)

layout(matrix(c(1, 1, 2, 3), ncol = 2), widths = 2.3:1, heights = 1:1)
...
#     legend(myPos, legend = CellType,
#          title = "Cell", pch = 17, col = myCol[1:length(CellType)],
#          cex = 1.2,
#          xpd = T,
#          lty = 1,
#          ncol = 1)
...
axis(2, at = seq(0, 150, length.out = 3), las = 1, cex.axis = 1.8)
axis(1, at = 1:8, 
     labels = unique(Data$Concentration),
     cex.axis = 1.8)


for (myF_concertion in c(10, 80)) {

  if (myF_concertion == 10) {
    par(mar = c(3, 0, 2, 3), mgp = c(2.5, 0.65, 0))
  }else if (myF_concertion == 80) {
    par(mar = c(5, 0, 0, 3), mgp = c(2.5, 0.65, 0))
  }

Data80 <- Data[which(Data$Concentration == myF_concertion),]
for (m in 2:length(CellType)) {
  Data80$Concentration[which(Data80$Cell == CellType[m])] <- Data80$Concentration[which(Data80$Cell == CellType[m])] + m-1
}

beeswarm(CV ~ Concentration, data = Data80, 
         col = myCol[1:length(CellType)], 
...
mean80Data <- mean80Data[match(CellType, mean80Data$Cell),]
mean80Data$x <- 1:length(CellType)
